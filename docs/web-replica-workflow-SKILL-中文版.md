...
> 8. **只评估当前 Project**：评估永远只针对当前复刻 project，不提供“评估全部”。
> 9. **只使用通用评估器**：评估器不内置固定目标，必须通过当前 project 配置调用：
>
> ```bash
> EVAL_TARGET_CONFIG=projects/{target-id}/config/target.json npm run eval
> ```
>
> 10. **只保留三个指标**：评估只包含功能一致性、交互一致性和视觉一致性。
> 11. **必须汇报访问地址**：每次复刻方案和交付更新都必须包含：
>
> ```text
...
🚧 **GATE**：阶段五完成；复刻页面可本地运行；`config/target.json` 指向当前 project。
