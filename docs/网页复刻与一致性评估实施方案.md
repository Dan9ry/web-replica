...
> 当前执行说明：本文档保留为早期总体实施方案与题目分析记录。实际复刻流程、project 目录约定、评估调用方式和交付清单，以项目内 `.codex/skills/web-replica-workflow/SKILL.md` 为准。新的复刻任务不再使用预置 `src/pages/*Replica/` 页面或固定 `evaluator/targets/*.json`，而是由 skill 为当前任务创建 `projects/{target-id}/`，再通过 `EVAL_TARGET_CONFIG=projects/{target-id}/config/target.json npm run eval` 调用通用评估器。
...
```text
本项目使用 AI 工具完成网页复刻，并通过自动化一致性评估框架量化比较原始页面与复刻页面在功能、交互、视觉上的一致程度。
```
...
> 原网页素材采集必须可信：如果原网页访问、截图、DOM 提取或关键元素识别失败，素材采集流程必须立即停止并输出错误原因，不能继续用主观猜测、空白截图、错误页或 AI 推断结果作为基线。评估阶段只读取已确认基线，不再交互式访问原站。

### 阶段目标

建立可配置、可自动运行、可输出报告的一致性评估框架。每个目标页面维护一个配置文件和一组 Phase 3 原站基线，评估脚本读取原站基线、采集本地复刻页面、计算指标并生成报告。
...
在任何一致性评分之前，必须先完成 Phase 3 原网页截图/DOM 基线校验。该校验是硬门禁，不通过则终止本次评估，不生成“看似有效”的分数。
...
1. `projects/{target-id}/config/target.json` 已列出所有需要复刻的真实状态。
2. 每个状态都有成功采集记录。
...
如果门禁失败，报告只展示“原网页基线门禁失败”的诊断信息，不展示一致性总分，也不允许把缺失或错误基线参与后续对比。
...
| 命令 | 用途 | 失败处理 |
| --- | --- | --- |
| `npm run eval` | 自动化/CI 评估 | 只读取当前 project 的 Phase 3 基线；基线缺失或不可信则中断 |
| `npm run eval:interactive` | 兼容旧命令 | 当前等同于 `npm run eval`，不再交互式打开原站 |

评估器使用 `projects/{target-id}/baselines/{state-id}/original-dom.json` 与 `original-{viewport}.png` 作为原站证据。验证码、AI 校验、安全验证等问题必须在 Phase 3 素材采集阶段解决；评估阶段发现基线缺失或不可信时，只能停止并提示返回素材采集阶段。
...
| 维度 | 权重 | 指标 | 计算方式 |
| --- | ---: | --- | --- |
| 功能一致性 | 35 | 核心功能覆盖率 | 已匹配复刻状态数 / 必需真实状态数 |
| 交互一致性 | 25 | 操作流程匹配度 | 已通过交互用例数 / 交互用例总数 |
| 视觉一致性 | 40 | 截图相似度、布局相似度、样式相似度 | SSIM、像素差异、元素位置差、颜色差 |
...
1. 读取 Phase 3 已确认的原站状态截图和 DOM 基线。
2. 执行原网页基线可信性门禁，确认不是空白页、错误页、拦截页或错误跳转页。
3. 将基线截图复制到 `reports/latest/assets/{page}/{state}/original-{viewport}.png`。
4. 打开本地复刻页面，按页面配置采集同名状态截图。
5. 对复刻页面状态注入稳定化脚本：暂停动画、隐藏光标、固定时间、关闭弹窗。
6. 保存每个复刻状态截图到 `reports/latest/assets/{page}/{state}/replica-{viewport}.png`。
7. 使用 `sharp` 统一尺寸。
...
- `npm run eval` 可以一键运行。
- 即使复刻页面还是空页面，也能在 Phase 3 原站基线可信后输出评分和问题列表。
- 如果原网页基线缺失或不可信，评估必须停止，并输出失败原因、最终 URL、缺失元素和基线路径。
...
| 模块 | 说明 |
| --- | --- |
| `projects/baidu/page/BaiduReplicaPage.tsx` | 页面容器，包含顶部链接区、Logo、搜索区、结果区 |
...
```text
请基于 Vite + React + TypeScript 实现一个百度首页复刻页面，只复刻搜索输入框、搜索按钮、搜索结果文案展示和分页交互。要求组件拆分清晰，样式尽量接近百度首页，搜索结果使用本地 mock 数据，不访问真实搜索接口。请把页面源码输出到 projects/baidu/page 下，使用 CSS Modules 样式。
```
...
| 模块 | 说明 |
| --- | --- |
| `projects/wechat-pay-login/page/WeChatPayLoginReplicaPage.tsx` | 页面整体布局 |
...
| Project 目录 | `projects/{target-id}/` |
| 页面源码 | 由当前 project 在 `projects/{target-id}/page/` 下创建 |
| 配置文件 | `projects/{target-id}/config/target.json` |
| E2E 文件 | `tests/e2e/third-page.spec.ts` |
| AI 记录 | `projects/{target-id}/logs/ai-log.md` |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `https://www.baidu.com` | 搜索框、按钮交互、结果展示、翻页 | `projects/baidu/page/` | 部署后填写 | 填写总分、等级、报告链接 |
| 2 | `https://pay.weixin.qq.com/index.php/core/home/login` | 用户名、密码、验证码输入、登录按钮交互 | `projects/wechat-pay-login/page/` | 部署后填写 | 填写总分、等级、报告链接 |
| 3 | 用户提供第三页 URL | 按用户指定范围填写 | `projects/{target-id}/page/` | 部署后填写 | 填写总分、等级、报告链接 |
...
在 `projects/baidu/page/` 下生成 `BaiduReplicaPage.tsx`、`SearchBox.tsx`、`SearchResults.tsx`、`Pagination.tsx` 和样式文件。
...
| 原网页基线缺失或不可信但继续评分 | 得到错误结论，影响评估可信度 | 设置原网页基线可信性门禁，失败立即停止，不生成一致性总分 |
