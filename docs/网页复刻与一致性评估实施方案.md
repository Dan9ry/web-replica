...
当 `npm run eval:baidu` 这类普通评估命令遇到安全验证、AI 校验、验证码或风控页时，不允许把当前错误页作为原站基准。评估器可以降级使用 `reports/baselines/{page}/{state}/` 中最近一次成功采集的原站截图和 DOM 摘要作为基准，并在报告中明确标记 `SOURCE_FALLBACK_SCREENSHOT`。若没有可用历史基准，则评估中断。

降级后只允许用历史原站基准做视觉/结构对比；交互一致性必须通过实时运行复刻页面流程获得，不能用原站验证页推断。所有低分维度都要在报告中给出修改建议，例如视觉差异图排查、搜索结果页布局修复、landmark 和可访问名称补齐等。

交互式评估补充：

| 命令 | 用途 | 失败处理 |
| --- | --- | --- |
| `npm run eval` | 自动化/CI 评估 | 验证问题优先使用最近一次成功基准降级；无基准则中断 |
